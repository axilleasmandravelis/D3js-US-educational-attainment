...
var color = d3.scaleLinear().domain([0, 1/4*5000000, 2/4*5000000, 3/4*5000000, 5000000]).range(["#5bada9", "#429b97", "#51ccc6", "#63f2eb"]);
...
d3.json("assets/data/us.json", function(data) {
    ...
    function display(d) {
        ...
        document.forms[0].addEventListener("change", function() {
            ...
            .sum(function (d) {
                if (treeSumSortType == "number") {
                    color = d3.scaleLinear().domain([0, 1/4*5000000, 2/4*5000000, 3/4*5000000, 5000000]).range(["#4c6809", "#618212", "#6f990c", "#8aba1a"]);
                    return d["Total College"];
                    ...
                    color = d3.scaleLinear().domain([0, 1/4*50, 2/4*50, 3/4*50, 50]).range(["#3e98a5", "#a59d2e", "#cec439", "#cec439"]);
                    return d["Percent College - Male"];
                } else {
                    color = d3.scaleLinear().domain([0, 1/4*50, 2/4*50, 3/4*50, 50]).range(["#592a58", "#511a4f", "#5e245c", "#753273"]);
